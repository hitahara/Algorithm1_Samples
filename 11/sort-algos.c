 ...
 * @brief レコードの最大長.
 */
#define MAX_RECORDS_SIZE 10000  // 1037499
/**
...
void q_sort_refined(sequence *seq, size_t left, size_t right) {
  if (left < right) {
    ...
    do {
      while (is_record_a_larger_than_b(pivot_record, seq->elements[i])) {
        i++;
      }
      while (is_record_a_larger_than_b(seq->elements[j], pivot_record)) {
        --j;
      }
      if (i <= j) {
...
void q_sort_definitive(sequence *seq, size_t whole_left, size_t whole_right) {
  ...
  do {
    ...
    while (right - left >= q_sort_limit) {
      ...
      j = right;
      do {
        for (; is_record_a_larger_than_b(pivot, seq->elements[i]); ++i)
          ;
        for (; is_record_a_larger_than_b(seq->elements[j], pivot); --j)
          ;
        if (i < j) {
          SWAP(record *, seq->elements[i], seq->elements[j])
        }
      } while (i < j);
      SWAP(record *, seq->elements[left + 1], seq->elements[j])

      if (j - left <= right - j) {
        push_stack(s, j + 1, right);
        right = j - 1;
      } else {
        push_stack(s, left, j - 1);
        left = j + 1;
      }
    }
  } while (s->sp > 0);
...
int main() {
  ...
  printf("Using algorithm: " STRINGFY(merge_sort) "\n");
  run_sort_algorithm(seq, merge_sort);
