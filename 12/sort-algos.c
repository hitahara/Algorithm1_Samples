...
void down_heap(sequence *seq, size_t target_node_index, size_t end_node_index);
void merge_sort(sequence *seq);
void merge_seqs(size_t segment_size, sequence *from, sequence *into);
void put(sequence *from, sequence *into, size_t *from_inputting_index, size_t *into_inputted_index);
void count_sort(sequence *seq, size_t m);
void bin_sort(sequence *seq, size_t m);
void radix_sort(sequence *seq, size_t radix_size);
size_t find_max_value(sequence *seq);
...
void heap_sort(sequence *seq) {
  for (size_t i = seq->elements_length / 2 - 1; i != (size_t)-1; i--) {
    down_heap(seq, i, seq->elements_length - 1);
  }

  for (size_t i = seq->elements_length - 1; i > 0;) {
...
void count_sort(sequence *seq, size_t m) {
  ...
  for (size_t i = seq->elements_length - 1; i != (size_t)-1; --i) {
    size_t j = counter[seq->elements[i]->key];
    j = (j - 1) % (size_t)-1;
    b[j] = seq->elements[i];
...
void bin_sort(sequence *seq, size_t m) {
  size_t list_head[m];
  size_t next[seq->elements_length];
  record *b[seq->elements_length];
  for (size_t i = 0; i < m; ++i) {
 ...
 */
void radix_sort(sequence *seq, size_t radix_size) {
  size_t max_value = find_max_value(seq);

  size_t next[seq->elements_length];
  size_t radix_digits_head[radix_size];
  record *b[seq->elements_length];

  for (size_t radix_digit = 1; radix_digit <= max_value; radix_digit *= radix_size) {
    ...
    for (size_t i = seq->elements_length - 1; i != (size_t)-1; --i) {
      ...
      size_t radix_digit_value = (seq->elements[i]->key / radix_digit) % radix_size;
...
size_t find_max_value(sequence *seq) {
  ...
  return max;  //(size_t)(log10(max) / log10(radix_size) + 1);
}
