 ...
 */
void linear_search_existence_and_index(table *tab, int target, bool *found, int *index) {
 ...
 */
void binary_search_existence_and_index(table *tab, int target, bool *found, int *index) {
  ...
  *index = lo;
  ...
  /*
  int ng = -1;
  int ok = tab->records_length; // tab->records[tab->records_length - 1].key < targetのことを考えて
  int mid;
  while (ok - ng > 1)
  {
    mid = (ok + ng) / 2;
...
void insert_record(table *tab, record *rec) {
  ...
  for (int i = tab->records_length; i > inserting_pos; i--) {
